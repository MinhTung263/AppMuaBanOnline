...
<body>
	...
	<div class="vertical-nav bg-white" id="sidebar">
		...
		<ul class="nav flex-column bg-white mb-0">
			...
			<li class="nav-item">
				<a href="index.php?server=DonDatHang" class="nav-link text-dark font-italic  bg-light">
	...
	<div class="page-content p-5" id="content">
		<table class="table">
						<thead class="thead-dark">
							<tr >
								<th id="tc" colspan="9"> + Đơn đặt hàng</th>
							</tr>
						</thead>
					...
					<?php 
					$stt=0;
					foreach ($od as $hang) { $stt++?>
						<tr>
							<td scope="col"><?='0'.$stt?></td>
							<td scope="col"><?=$hang["MaDonHang"]?></td>
							<td scope="col" ><h6  class="truncate "> <?=$hang["TenKhach"]?></h6></td>
							<td scope="col"><?=date("d/m/Y",strtotime($hang["NgayDat"]))?></td>
							<td scope="col"><h6  class="truncate "> <?=$hang["TenHang"]?></h6></td>
							<td scope="col"><img  width=40px ;height=40px src="<?=$hang["HinhAnh"]?>"></td>
							<td scope="col"><h6 class="truncate "><?=$hang["SoLuong"]?></h6> </td>
							<td scope="col"><?=number_format($hang["DonGia"]*$hang["SoLuong"],0).' VNĐ'?></td>
							<td scope="col"><a href="index.php?server=Xoa&id=<?=$hang['MaHang']?>">Lập đơn hàng /</a>
